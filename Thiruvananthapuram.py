...
df = pd.read_csv('finalwithorder.csv')
df.dropna(inplace=True)
df.reset_index(drop=True,inplace=True)
df = df[df['Datetime'].str.contains('2020')]
df = df.append(df[df['Datetime'].str.contains('2019')])
enc = OrdinalEncoder()
...
df.drop(df[df['Temperature'] > 88 ].index, inplace = True)
Thiruvananthapuram_dataset = df[df['Datetime'].str.contains('2020')].copy()
df = df.drop(['Precip.','Datetime'], axis=1)
df.reset_index(drop=True,inplace=True)
df_Haze = df[df['Condition'] == 'Haze']
df_Cloudy = df[df['Condition'] == 'Mostly Cloudy']
df_Fog = df[df['Condition'] == 'Fog']
df_T_Storm = df[df['Condition'] == 'Thunder']
...
df_new.drop(['Wind Gust','Wind'], axis=1, inplace=True)
...
def Thiruvananthapuram_KNC(lst):
...
def Thiruvananthapuram_DTC(lst):
...
def Thiruvananthapuram_RFC(lst):
...
def Thiruvananthapuram_SVC(lst):
...
def Thiruvananthapuram_ANN(lst):
...
def Thiruvananthapuram_line_chart(model_temp,model_dew,model_hum,model_wind_sp,model_press):
    Thiruvananthapuram_dataset['Month'] = Thiruvananthapuram_dataset['Datetime'].apply(changedate)
    Thiruvananthapuram_dataset['Month'] = Thiruvananthapuram_dataset['Month'].astype(int)
    Thiruvananthapuram_dataset.drop_duplicates(subset=['Month'], inplace=True)
    Thiruvananthapuram_dataset.set_index(['Month'], inplace=True)
    ...
    df3 = pd.concat([Thiruvananthapuram_dataset, df2], ignore_index=True)
